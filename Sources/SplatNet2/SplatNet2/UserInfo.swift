...
public struct UserInfo: Codable, Hashable, Identifiable {
    ...
    mutating func update(from response: SummaryCoop.Response) {
        coop.jobNum = response.summary.card.jobNum
        coop.goldenIkuraTotal = response.summary.card.goldenIkuraTotal
        coop.ikuraTotal = response.summary.card.ikuraTotal
        coop.kumaPoint = response.summary.card.kumaPoint
        coop.kumaPointTotal = response.summary.card.kumaPointTotal
    }
}
